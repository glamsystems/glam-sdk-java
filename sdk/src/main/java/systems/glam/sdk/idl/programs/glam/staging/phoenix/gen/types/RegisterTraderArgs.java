package systems.glam.sdk.idl.programs.glam.staging.phoenix.gen.types;

import software.sava.idl.clients.core.gen.SerDe;

import static software.sava.core.encoding.ByteUtil.getInt64LE;
import static software.sava.core.encoding.ByteUtil.putInt64LE;

public record RegisterTraderArgs(long maxPositions,
                                 int traderPdaIndex,
                                 int subaccountIndex) implements SerDe {

  public static final int BYTES = 10;

  public static final int MAX_POSITIONS_OFFSET = 0;
  public static final int TRADER_PDA_INDEX_OFFSET = 8;
  public static final int SUBACCOUNT_INDEX_OFFSET = 9;

  public static RegisterTraderArgs read(final byte[] _data, final int _offset) {
    if (_data == null || _data.length == 0) {
      return null;
    }
    int i = _offset;
    final var maxPositions = getInt64LE(_data, i);
    i += 8;
    final var traderPdaIndex = _data[i] & 0xFF;
    ++i;
    final var subaccountIndex = _data[i] & 0xFF;
    return new RegisterTraderArgs(maxPositions, traderPdaIndex, subaccountIndex);
  }

  @Override
  public int write(final byte[] _data, final int _offset) {
    int i = _offset;
    putInt64LE(_data, i, maxPositions);
    i += 8;
    _data[i] = (byte) traderPdaIndex;
    ++i;
    _data[i] = (byte) subaccountIndex;
    ++i;
    return i - _offset;
  }

  @Override
  public int l() {
    return BYTES;
  }
}
