...
final class MinGlamStateAccountTests {
  ...
  private static final String STATE_B64 = """
  ...
  static byte[] fixtureData() {
    return Base64.getDecoder().decode(STATE_B64);
  }

  @Test
  void minStateAccountSerialization() {
    final byte[] stateAccountData = fixtureData();
...
//    assertArrayEquals(minStateAccount.assetBytes(), changed.assetBytes());
  ...
  /// the previous record's arrays for the rest — by identity, not by copy.
  /// Content assertions cannot tell a reuse from a reparse, so these pin the
  /// instances.
  @Test
  void createIfChangedReusesUnchangedSectionsByIdentity() {
    final byte[] data = fixtureData();
    final var stateAccount = StateAccount.read(STATE_ACCOUNT_KEY, data);
    final var minStateAccount = MinGlamStateAccount.createRecord(accountInfo(100L, data));

    // the enabled flag lives outside every section: flipping it alone must
    // produce a change that reuses all four section arrays untouched
    final byte[] disabledData = data.clone();
    assertEquals(1, disabledData[StateAccount.ENABLED_OFFSET], "fixture should be enabled");
    disabledData[StateAccount.ENABLED_OFFSET] = 0;
    final var disabled = minStateAccount.createIfChanged(accountInfo(101L, disabledData));
    assertNotNull(disabled);
    assertTrue(disabled.disabled());
    assertSame(minStateAccount.assets(), disabled.assets());
    assertSame(minStateAccount.protocolIntegrations(), disabled.protocolIntegrations());
    assertSame(minStateAccount.delegates(), disabled.delegates());
    assertSame(minStateAccount.externalPositions(), disabled.externalPositions());

    // a changed integration ACL reparses that section and only that section
    final var changedAcls = stateAccount.integrationAcls().clone();
    final var firstAcl = changedAcls[0];
    changedAcls[0] = new systems.glam.sdk.idl.programs.glam.protocol.gen.types.IntegrationAcl(
        firstAcl.integrationProgram(), firstAcl.protocolsBitmask() ^ 1, firstAcl.protocolPolicies()
    );
    final var changedIntegrations = new StateAccount(
        stateAccount._address(), stateAccount.discriminator(), stateAccount.accountType(),
        stateAccount.enabled(), stateAccount.vault(), stateAccount.owner(),
        stateAccount.portfolioManagerName(), stateAccount.created(), stateAccount.baseAssetMint(),
        stateAccount.baseAssetDecimals(), stateAccount.baseAssetTokenProgram(), stateAccount.name(),
        stateAccount.timelockDuration(), stateAccount.timelockExpiresAt(), stateAccount.mint(),
        stateAccount.assets(), changedAcls, stateAccount.delegateAcls(),
        stateAccount.externalPositions(), stateAccount.pricedProtocols(), stateAccount.params()
    );
    final var integrationsChanged = minStateAccount.createIfChanged(accountInfo(102L, changedIntegrations.write()));
    assertNotNull(integrationsChanged);
    assertNotSame(minStateAccount.protocolIntegrations(), integrationsChanged.protocolIntegrations());
    assertSame(minStateAccount.assets(), integrationsChanged.assets());
    assertSame(minStateAccount.delegates(), integrationsChanged.delegates());
    assertSame(minStateAccount.externalPositions(), integrationsChanged.externalPositions());
    assertEquals(minStateAccount.enabled(), integrationsChanged.enabled());
  }

  @Test
  void immutableBaseFieldsChangingIsAnError() {
    final byte[] data = fixtureData();
    final var minStateAccount = MinGlamStateAccount.createRecord(accountInfo(100L, data));

    // base asset decimals feed share-price math; a change is corruption, not
    // an update
    final byte[] changedDecimals = data.clone();
    changedDecimals[StateAccount.BASE_ASSET_DECIMALS_OFFSET] ^= 1;
    final var decimalsError = assertThrows(
        IllegalStateException.class,
        () -> minStateAccount.createIfChanged(accountInfo(101L, changedDecimals))
    );
    assertTrue(decimalsError.getMessage().contains("decimals"), decimalsError.getMessage());

    final byte[] changedBaseAsset = data.clone();
    changedBaseAsset[StateAccount.BASE_ASSET_MINT_OFFSET] ^= 1;
    final var baseAssetError = assertThrows(
        IllegalStateException.class,
        () -> minStateAccount.createIfChanged(accountInfo(101L, changedBaseAsset))
    );
    assertTrue(baseAssetError.getMessage().contains("Base asset changed"), baseAssetError.getMessage());
  }
}
