...
public interface VaultPriceService extends DelegateService {
  ...
  static VaultPriceService createService(final ServiceContext serviceContext,
                                         ...
                                         final MinStateAccount minStateAccount) {
    final var glamClient = GlamAccountClient.createClient(serviceContext.serviceKey(), stateAccountKey);
    final var accountsNeeded = HashSet.<PublicKey>newHashSet((2 + (minStateAccount.numAccounts() << 2)));
    accountsNeeded.add(stateAccountKey);
    accountsNeeded.add(baseAssetMint);
    for (final var asset : minStateAccount.assets()) {
      if (integContext.mintContext(asset) == null) {
        accountsNeeded.add(asset);
      }
    }
    for (final var externalPosition : minStateAccount.externalPositions()) {
      accountsNeeded.add(externalPosition);
    }

    return new MultiAssetPriceService(
        serviceContext, integContext,
        baseAssetMint, baseAssetDecimals, glamClient, minStateAccount,
        accountsNeeded
    );
  }
  ...
  static VaultPriceService createService(final ServiceContext serviceContext,
    ...
    final var minStateAccount = MinStateAccount.createRecord(stateAccountInfo);
  ...
  static VaultPriceService createService(final ServiceContext serviceContext,
    ...
    final var minStateAccount = new MinStateAccount(slot, assetBytes, assets, externalPositionsBytes, externalPositions);
